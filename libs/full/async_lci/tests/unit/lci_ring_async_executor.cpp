...
#include <hpx/modules/async_lci.hpp>
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    {
        // this needs to scope all uses of hpx::lci::experimental::executor
        hpx::lci::experimental::enable_user_polling enable_polling;

        // Ring send/recv around N ranks
        // Rank 0      : Send then Recv
        // Rank 1->N-1 : Recv then Send

        hpx::lci::experimental::executor exec(MPI_COMM_WORLD);

        // lci chokes if we put too many messages into the system at once
        // we will use a limiting executor with N 'in flight' at once
        hpx::execution::experimental::limiting_executor<
            hpx::lci::experimental::executor>
        ...
        hpx::lci::experimental::wait([&]() { return counter != 0; });
