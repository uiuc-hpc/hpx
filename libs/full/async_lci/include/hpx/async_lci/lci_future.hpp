...
namespace hpx { namespace lci { namespace experimental {
    ...
    static constexpr print_on lci_debug("MPI_FUT");
    ...
    namespace detail {
        ...
        // too many lci requests are being spawned at once
    ...
    // Checks for completed MPI_Requests and sets lci::experimental::future ready
    // when found
    ...
    HPX_EXPORT void init(bool init_lci = false,
    ...
    struct HPX_NODISCARD enable_user_polling
    {
        ...
        {
            lci::experimental::init(false, pool_name);
        }

        ~enable_user_polling()
        {
            lci::experimental::finalize(pool_name_);
        }
    ...
    {
        lci_debug.debug(detail::get_lci_info(), std::forward<Args>(args)...);
    }
