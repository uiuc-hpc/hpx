...
#include <hpx/async_lci/lci_future.hpp>
#include <hpx/modules/errors.hpp>
#include <hpx/modules/threading_base.hpp>
#include <hpx/lci_base/lci_environment.hpp>
...
namespace hpx { namespace lci { namespace experimental {
    namespace detail {
        ...
        // mutex needed to protect lci request vector, note that the
        // lci poll function takes place inside the main scheduling loop
        ...
        MPI_Errhandler hpx_lci_errhandler = 0;

        // an instance of lci_info that we store data in
        lci_info& get_lci_info()
        {
            static lci_info lci_info_;
            return lci_info_;
        }

        // stream operator to display debug lci_info
        HPX_EXPORT std::ostream& operator<<(std::ostream& os, lci_info const&)
        {
            os << "R " << debug::dec<3>(get_lci_info().rank_) << "/"
               << debug::dec<3>(get_lci_info().size_);
            return os;
        }
        ...
        {
            lci_debug.debug(debug::str<>("hpx_MPI_Handler"));
        ...
        void add_to_request_queue(future_data_ptr data)
        {
            ...
            lci_debug.debug(debug::str<>("request queued"), get_lci_info(),
        ...
        void add_to_request_vector(future_data_ptr data)
        {
            ...
            lci_debug.debug(debug::str<>("push_back"), get_lci_info(),
                "req_ptr", debug::ptr(get_active_requests().data()));

            lci_debug.debug(debug::str<>("add request"), get_lci_info(),
    ...
    {
        lci_debug.debug(debug::str<>("set_error_handler"));

        MPI_Comm_create_errhandler(
            detail::hpx_MPI_Handler, &detail::hpx_lci_errhandler);
        MPI_Comm_set_errhandler(MPI_COMM_WORLD, detail::hpx_lci_errhandler);
    }

    // Background progress function for MPI async operations
    // Checks for completed MPI_Requests and sets lci::experimental::future
    // ready when found
    void poll()
    {
        ...
        if (!lk.owns_lock())
        {
            if (lci_debug.is_enabled())
            {
                // for debugging
                static auto poll_deb = lci_debug.make_timer(1,
                    debug::str<>("Poll - lock failed"), detail::get_lci_info());

                lci_debug.timed(poll_deb, "requests",
        ...
        {
            // for debugging
            static auto poll_deb = lci_debug.make_timer(
                1, debug::str<>("Poll - lock success"), detail::get_lci_info());
        ...
        while (keep_trying)
        {
            ...
            if (result == MPI_SUCCESS)
            {
                ...
                {
                    static auto poll_deb = lci_debug.make_timer(1,
                        debug::str<>("Poll - success"), detail::get_lci_info());

                    // clang-format off
                    lci_debug.timed(poll_deb,
                        debug::str<>("Success"),
                ...
                if (keep_trying)
                {
                    ...
                    lci_debug.debug(debug::str<>("MPI_Testany(set)"),
                        detail::get_lci_info(), "request", debug::hex<8>(req),
                        "vector size",
            ...
            else
            {
                ...
                if (lci_debug.is_enabled())
                {
                    auto poll_deb = lci_debug.make_timer(1,
                        debug::str<>("Poll - <ERR>"), detail::get_lci_info());

                    // clang-format off
                    lci_debug.error(poll_deb,
                        debug::str<>("Poll <ERR>"),
            ...
            if (nulls > detail::get_active_requests().size() / 4)
            {
                    ...
                    detail::get_active_futures().size())
                {
                    HPX_THROW_EXCEPTION(invalid_status,
                        "hpx::lci::experimental::poll",
                        "Fatal Error: Mismatch in vectors");
                }

                lci_debug.debug(debug::str<>("MPI_REQUEST_NULL"),
                    detail::get_lci_info(), "vector size",
                    debug::dec<3>(detail::get_active_futures().size()),
...
#endif
            lci_debug.debug(debug::str<>("enable polling"));
            auto* sched = pool.get_scheduler();
            sched->set_mpi_polling_function(&hpx::lci::experimental::poll);
        }
...
#if defined(HPX_DEBUG)
            {
                std::unique_lock<hpx::lci::experimental::detail::mutex_type> lk(
...
#endif
            lci_debug.debug(debug::str<>("disable polling"));
            auto* sched = pool.get_scheduler();
            sched->clear_mpi_polling_function();
        }
    }    // namespace detail

    // initialize the hpx::lci background request handler
    // All ranks should call this function,
    // but only one thread per rank needs to do so
    void init(
        bool init_lci, std::string const& pool_name, bool init_errorhandler)
    {
        if (init_lci)
        {
            ...
            {
                lci_debug.error(debug::str<>("hpx::lci::experimental::init"),
                    "init failed");
                HPX_THROW_EXCEPTION(invalid_status,
                    "hpx::lci::experimental::init",
                    "the MPI installation doesn't allow multiple threads");
            }
            MPI_Comm_rank(MPI_COMM_WORLD, &detail::get_lci_info().rank_);
            MPI_Comm_size(MPI_COMM_WORLD, &detail::get_lci_info().size_);
        }
        else
        {
            // Check if MPI_Init has been called previously
            if (detail::get_lci_info().size_ == -1)
            {
                ...
                {
                    MPI_Comm_rank(
                        MPI_COMM_WORLD, &detail::get_lci_info().rank_);
                    MPI_Comm_size(
                        MPI_COMM_WORLD, &detail::get_lci_info().size_);
                }
            }
        }

        lci_debug.debug(debug::str<>("hpx::lci::experimental::init"),
            detail::get_lci_info());

        if (init_errorhandler)
        {
            set_error_handler();
            detail::get_lci_info().error_handler_initialized_ = true;
        }
    ...
    {
        if (detail::get_lci_info().error_handler_initialized_)
        {
            HPX_ASSERT(detail::hpx_lci_errhandler != 0);
            detail::get_lci_info().error_handler_initialized_ = false;
            MPI_Errhandler_free(&detail::hpx_lci_errhandler);
            detail::hpx_lci_errhandler = 0;
        }

        // clean up if we initialized lci
        hpx::util::lci_environment::finalize();

        lci_debug.debug(debug::str<>("Clearing mode"), detail::get_lci_info(),
...
}}}    // namespace hpx::lci::experimental
