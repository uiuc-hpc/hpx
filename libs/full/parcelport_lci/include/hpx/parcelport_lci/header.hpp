    ...
    struct header
    {
            ...
            char* header_buffer, size_t max_header_size) noexcept
        {
            ...
            if (buffer.data_.size() <= (max_header_size - current_header_size))
            {
                ...
                std::memcpy(&data_[current_header_size], &buffer.data_[0],
                    size);
        ...
        header(char* header_buffer) noexcept {
            data_ = header_buffer;
        }
        ...
        {
            if (data_[pos_piggy_back_flag_data] || data_[pos_piggy_back_flag_tchunk])
